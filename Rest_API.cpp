...
//GET - /api/config
std::string HomeAssistant::GET_api_config(){
    CURL *curl;
    struct curl_slist *list = NULL;
    CURLcode res;
    ...
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_URL, &url[0]);
        //curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L); //Follow redirects
        //curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, HomeAssistant::WriteCallback);
        //curl_easy_setopt(curl, CURLOPT_WRITEDATA, &HomeAssistant::readBuffer);
        list = curl_slist_append(list, &auth[0]);
        list = curl_slist_append(list, "Content-Type: application/json");
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, list);
        res = curl_easy_perform(curl);
